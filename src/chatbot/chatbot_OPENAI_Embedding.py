...
# ===EMBEDDING===
embeddings = OpenAIEmbeddings(model="text-embedding-3-large", openai_api_key=OPEN_API_KEY,
...
if os.path.exists(VECTOR_DB_DEMO_PATH):
    ...
    vector_store = FAISS.load_local(VECTOR_DB_DEMO_PATH,
                                    embeddings,
    ...
    vector_store.save_local(VECTOR_DB_DEMO_PATH)
...
response = graph.invoke({"question": "Hướng dẫn tôi kết nối facebook trên App Sapo"})
